...
namespace Ioke.Math {
    ...
    public class IntNum : RatNum {
        public int ival;
        public int[] words;


        /** We pre-allocate integers in the range minFixNum..maxFixNum. */
        const int minFixNum = -100;
        const int maxFixNum = 1024;
        const int numFixNum = maxFixNum-minFixNum+1;
        static readonly IntNum[] smallFixNums = new IntNum[numFixNum];
        ...
        public static IntNum zero ()
        {
            return smallFixNums[- minFixNum];
        }

        public static IntNum one ()
        {
            return smallFixNums[1 - minFixNum];
        }

        public static IntNum ten ()
        {
                     ...
                     || words.Length > nwords + 2)
                {
                    ...
                    if (words == null)
                    ...
                    else
                        {
                            if (nwords < ival)
                                ival = nwords;
                            Array.Copy(words, new_words, ival);
                        }
                    words = new_words;
                }
        }

        public override IntNum numerator ()
        {
            return this;
        }

        public override IntNum denominator ()
        {
            return one ();
        }
        ...
        public void set (int[] words, int length)
        {
            this.ival = length;
            this.words = words;
        }
        ...
        public void set (IntNum y)
        {
            if (y.words == null)
                ...
                {
                    realloc(y.ival);
                    Array.Copy(y.words, words, y.ival);
                    ival = y.ival;
                }
        }
        ...
        public static IntNum times (int x, int y)
        {
            return IntNum.make ((long) x * (long) y);
        }
        ...
        public override RealNum toInt (int rounding_mode)
        {
            return this;
        }
            ...
            for (int i = 0;  i < length;  i++)
                {
                    ...
                    if (ch == '-')
                        ...
                        {
                            int digit = digitForChar(ch, radix);
                            if (digit < 0)
                                break;
        ...
        public double roundToDouble (int exp, bool neg, bool remainder)
        {
            // Compute length.
            int il = intLength();
         ...
         * Ok for src==dest. */
        public static bool negate (int[] dest, int[] src, int len)
        {
            long carry = 1;
        ...
        public override decimal AsDecimal() {
            if(words == null) {
                return System.Convert.ToDecimal(ival);
            }

            if(ival <= 2) {
                return System.Convert.ToDecimal(longValue());
            }

            return System.Convert.ToDecimal(ToString());
        }
