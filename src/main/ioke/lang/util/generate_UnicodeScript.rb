...
SCRIPTS = {"Unknown" => 0xFF00}
i = 0xF000
MAP.values.sort {|a, b| a.to_s <=> b.to_s }.uniq.each {|k| i += 1; SCRIPTS[k.to_s] = i }
...
SCRIPTS.keys.sort.each {|k| printf "  %s('\\u%04x'),\n", k, SCRIPTS[k.to_s] }
puts %q[
  ;
  
  private final char order;
  
  UnicodeScript(char order) {
    this.order = order;
  }
  
  public static UnicodeScript getScriptFor(int codePoint) {
    for(UnicodeScript s : values()) {
      if(s.order == DATA.charAt(codePoint)) {
        return s;
      }
    }
    return null;
...
print '  private static final String DATA = new StringBuilder("'
lookup.each_with_index do |s, i|
  printf '\\u%04x', SCRIPTS[s.to_s]
  print "\")\n    .append(\"" if i % 256 == 0
end
puts '").toString();'
puts "\n}"
