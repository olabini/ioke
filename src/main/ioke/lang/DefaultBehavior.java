...
public class DefaultBehavior {
    public static void init(IokeObject obj) {
        ...
        obj.registerMethod(new JavaMethod(runtime, "", "returns result of evaluating first argument") {
        ...
        obj.registerMethod(new JavaMethod(runtime, "derive", "calls mimic.") {
                public IokeObject activate(IokeObject context, Message message, IokeObject on) throws ControlFlow {
                    return runtime.base.getCell(message, context, "mimic").activate(context, message, on);
                }
            });

        obj.registerMethod(new JavaMethod(runtime, "break", "breaks out of the enclosing context. if an argument is supplied, this will be returned as the result of the object breaking out of") {
                public IokeObject activate(IokeObject context, Message message, IokeObject on) throws ControlFlow {
        ...
        obj.registerMethod(new JavaMethod(runtime, "until", "until the first argument evaluates to something true, loops and evaluates the next argument") {
                public IokeObject activate(IokeObject context, Message message, IokeObject on) throws ControlFlow {
                    if(message.getArgumentCount() == 0) {
                        return runtime.nil;
                    }
        ...
        obj.registerMethod(new JavaMethod(runtime, "while", "while the first argument evaluates to something true, loops and evaluates the next argument") {
                public IokeObject activate(IokeObject context, Message message, IokeObject on) throws ControlFlow {
                    if(message.getArgumentCount() == 0) {
                        return runtime.nil;
                    }
        ...
        obj.registerMethod(new JavaMethod(runtime, "loop", "loops forever - executing it's argument over and over until interrupted in some way.") {
                public IokeObject activate(IokeObject context, Message message, IokeObject on) throws ControlFlow {
                    if(message.getArgumentCount() > 0) {
                        try {
                            while(true) {
        ...
        obj.registerMethod(new JavaMethod(runtime, "if", "evaluates the first arguments, and then evaluates the second argument if the result was true, otherwise the last argument. returns the result of the call, or the result if it's not true.") {
                public IokeObject activate(IokeObject context, Message message, IokeObject on) throws ControlFlow {
        ...
        obj.registerMethod(new JavaMethod(runtime, "internal:createText", "expects one 'strange' argument. creates a new instance of Text with the given Java String backing it.") {
        ...
        obj.registerMethod(new JavaMethod(runtime, "++", "expects one argument, which is the unevaluated name of the cell to work on. will retrieve the current value of this cell, call 'succ' to that value and then send = to the current receiver with the name and the resulting value.") {
                public IokeObject activate(IokeObject context, Message message, IokeObject on) throws ControlFlow {
