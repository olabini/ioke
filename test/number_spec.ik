...
describe(Number,
  ...
  describe("sqrt",
    ...
    it("should work for really large natural numbers",
      6019435340558886869880625 sqrt should == 2453453757575
      754602587682178164614184239907866122868939846675876839608496471649499866413576215856872728460087288314631620935936 sqrt should be close(868678644656456467567567567654268954869458694589548969456.0)
    )

    it("should work for really large decimal numbers",
      part = 34711111245645457567.253456363
      (part*part) sqrt should == part

      part = 868692345345346356889489345893457718917243897534987593874569345698928372948579283457398546793892848273523.34634563576357467568585856
      (part*part) sqrt should be close(part)
    )

    it("should work for really small ratios",
      (9/12500943659355365449) sqrt should == 3/3535667357

      (7921/3762183068788232642790328892777442567701717176782637147984615156218140259073516437317309980367754652967893287474312549921167385631521691523841071577742887635712392592340464009112448017845497797009) sqrt should == 356/245346548988592301912742874383753959385948549469247245019391728418472935742352358972395872359823012
    )
  ...
  describe(Number ∞,
    it("should have the correct kind",
      Number ∞ kind should == "Number ∞"
    )

    it("should mimic Ratio",
      Number ∞ should mimic(Number Ratio)
    )

    it("should be represented with a descriptive string",
      Number ∞ notice should == "∞"
    )

    describe("inspect",
      it("should validate type of receiver",
        Number ∞ should checkReceiverTypeOn(:"inspect")
        x = Origin mimic
        x cell("inspect") = Number ∞ cell("inspect")
        fn(x inspect) should signal(Condition Error Type IncorrectType)
      )
    )
    ...
    describe("notice",
      it("should validate type of receiver",
        Number ∞ should checkReceiverTypeOn(:"notice")
      )
    )

    describe("<",
      it("should not be less than, well, anything",
        (Number ∞ < 1) should not be true
      )
    )

    describe(">",
      it("should be greater than, well, everything",
        (Number ∞ > 31489014839014839014893014839018439184903184390184310943104391048390184309) should be true
      )
    )
    ...
    describe("==",
      ...
      it("should be inequal to another rational",
        Number ∞ should not == 42
      )

      it("should be inequal to a decimal",
        Number ∞ should not == 3.3
      )
    )

    describe("mimic",
      it("cannot be mimiced",
        fn(Number ∞ mimic) should signal(Condition Error CantMimicOddball)
      )
    )
  )
)
